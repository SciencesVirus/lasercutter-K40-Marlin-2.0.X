...
void GCodeQueue::RingBuffer::ok_to_send()
{
  ...
  if (!serial_ind.valid())
    return;                                   // Optimization here, skip processing if it's not going anywhere
...
#if HAS_MULTI_SERIAL
  if (!serial_ind.valid())
    return;                                   // Optimization here, skip if the command came from SD or Flash Drive
  PORT_REDIRECT(SERIAL_PORTMASK(serial_ind)); // Reply to the serial port that sent the command
#endif
  SERIAL_FLUSH();
...
void GCodeQueue::gcode_line_error(PGM_P const err, const serial_index_t serial_ind)
{
  ...
  while (read_serial(serial_ind) != -1)
  { /* nada */
  } // Clear out the RX buffer. Why don't use flush here ?
...
void GCodeQueue::report_buffer_statistics()
{
  SERIAL_ECHOLNPGM("D576"
                   " P:",
                   planner.moves_free(), " ", -queue.planner_buffer_underruns, " (", queue.max_planner_buffer_empty_duration, ")"
                                                                                                                              " B:",
                   BUFSIZE - ring_buffer.length, " ", -queue.command_buffer_underruns, " (", queue.max_command_buffer_empty_duration, ")");
...
void GCodeQueue::auto_report_buffer_statistics()
{
  ...
  if (planner.movesplanned() == 0)
  {
    ...
    { // the planner buffer wasn't empty, but now it is
      planner_buffer_empty = true;
      planner_buffer_underruns++;
      planner_buffer_empty_at = ms;
    }
  }
  else if (planner_buffer_empty)
  { // the planner buffer was empty, but now it's not
